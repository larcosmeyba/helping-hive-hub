...
export default function DashboardHome() {
  ...
  return (
    <div className="w-full max-w-[360px] md:max-w-6xl mx-auto space-y-2.5 md:space-y-6 overflow-x-hidden px-1 md:px-0">
      {/* Welcome */}
      <div className="space-y-1.5">
        ...
        <Button
          onClick={generate}
          disabled={generating}
          className="bg-gradient-honey text-primary-foreground hover:opacity-90 h-6 text-[9px] px-2.5 md:text-sm md:h-9 md:px-4 w-[220px] md:w-auto"
        >
          {generating ? (
      ...
      {/* Stats Grid */}
      <div className="grid grid-cols-4 gap-1 md:gap-4 max-w-[300px] md:max-w-none">
        {[
          ...
          <div key={stat.label} className="bg-card rounded-md border border-border p-1 md:p-4 shadow-card aspect-square">
            ...
            <p className="text-[9px] md:text-2xl font-bold text-foreground">{stat.value}</p>
          </div>
        ))}

        <Link
          to="/dashboard/pantry"
          className="group overflow-hidden bg-gradient-to-br from-accent/10 to-primary/10 rounded-md border border-accent/20 p-1 md:p-4 shadow-card hover:shadow-lg hover:border-accent/40 transition-all duration-300 aspect-square"
        >
          <div className="flex items-center gap-0.5 mb-0.5">
        ...
        <div>
          ...
          {mealPlan.weeklyPlan.slice(0, 2).map((day) => (
            <div key={day.day} className="mb-2 md:mb-6">
              ...
              <div
                className="flex gap-1.5 overflow-x-auto scrollbar-hide snap-x snap-mandatory pb-1 -mx-1 px-1 md:grid md:grid-cols-3 md:gap-3 md:overflow-visible md:pb-0 md:mx-0 md:px-0"
          ...
          <Link
            ...
            className="inline-flex items-center justify-center w-[110px] bg-primary/5 hover:bg-primary/10 border border-primary/20 rounded h-5 md:h-auto md:w-auto md:py-1 md:px-3 text-[8px] md:text-sm font-semibold text-primary transition-colors"
          >
            View Full Plan →
          </Link>
        </div>

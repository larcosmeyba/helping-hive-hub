...
export function CookFromFridge() {
  ...
  return (
    <div className="bg-gradient-to-br from-card to-secondary/30 rounded-lg border border-border p-2 md:p-6 shadow-card w-full max-w-[340px] md:max-w-none">
      <div className="flex items-center gap-1.5 mb-2 md:mb-4">
        ...
        <div>
          <h2 className="font-display text-[11px] md:text-lg font-semibold text-foreground">Fridge Chef</h2>
      ...
      <div className="flex flex-wrap gap-1 md:gap-2 mb-2 md:mb-4">
        {COMMON_ITEMS.map((item) => (
          <button
            key={item}
            onClick={() => toggle(item)}
            className={`px-1.5 py-0.5 md:px-3 md:py-1.5 rounded-full text-[7px] md:text-xs font-medium border transition-all ${
      ...
      {selectedItems.filter((i) => !COMMON_ITEMS.includes(i)).length > 0 && (
        <div className="flex flex-wrap gap-1 mb-2">
          {selectedItems.filter((i) => !COMMON_ITEMS.includes(i)).map((item) => (
            <span key={item} className="flex items-center gap-0.5 bg-accent/10 text-accent text-[7px] md:text-xs font-medium px-1.5 py-0.5 rounded-full">
              {item}
      ...
      <div className="flex items-center gap-1.5 min-w-0">
        <Link
          to={`/dashboard/pantry`}
          className="text-[8px] md:text-xs text-muted-foreground hover:text-primary underline whitespace-nowrap"
        >
          Manage Pantry →
        </Link>
        <Button
          ...
          className="bg-gradient-honey text-primary-foreground hover:opacity-90 ml-auto h-6 text-[8px] px-2 md:h-9 md:text-sm md:px-4 whitespace-nowrap"
          size="sm"
        >
